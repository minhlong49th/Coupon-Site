...
import { SubmitCodeButton } from "@/components/public/SubmitCodeButton";
import { SimilarStores } from "@/components/public/SimilarStores";
...
export async function generateMetadata({
  ...
  if (!brandParam.endsWith("-coupon-codes")) {
    return { title: "Not Found" };
  }
  const brandSlug = brandParam.slice(0, -"-coupon-codes".length);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 w-full min-w-0">
      <div className="text-xs text-gray-500 mb-4">
        Home / {brand.category} /{" "}
        <span className="text-gray-900 font-medium">{brand.name} Coupons</span>
      </div>
      ...
      <div className="bg-white border-b border-gray-200 py-8 px-6 rounded-2xl mb-8 flex flex-col md:flex-row gap-6 items-start md:items-center">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
            ...
            <span className="text-xs bg-violet-50 text-violet-700 px-2 py-1 rounded-full font-bold uppercase tracking-wider">
              {brand.category}
            </span>
          </h1>
          <TrackedLink
            ...
            brandId={brand.id}
            className="text-sm text-gray-500 hover:text-violet-600 mt-1 inline-block"
          >
            {brand.domain}
          </TrackedLink>
        </div>
        ...
        <div className="lg:w-1/3 space-y-6">
          <div className="bg-violet-50 border border-violet-100 rounded-xl p-5 shadow-sm">
            <h3 className="font-bold text-violet-900 mb-2">
              Know a code for {brand.name}?
            </h3>
            <p className="text-violet-700 text-sm mb-4">
              Share it with our community and earn karma points for every
              working code.
            </p>
            <SubmitCodeButton
          ...
          <SimilarStores similarBrands={similarBrands} />
        </div>
      </div>
    </div>
  );
